...
The Application is an unofficial fan-made collection guide app for Animal Crossing: New Horizons.  
The Application is not affiliated with, endorsed by, or sponsored by Nintendo.  
All related game assets, characters, and trademarks are the property of their respective owners.

The Application does not require user registration and does not operate its own servers.  
All user-created data (such as collection progress, island information, images, and settings) is stored locally on the user's device.
...
Images captured or selected by the user are stored locally on the device.  
The Developer does not collect, upload, transmit, store externally, or access these images.
...
All collection progress, island information, preferences, and user-created data within the Application are stored locally on the user's device.  
...
The Application is not specifically directed to children under the age of 13.  
The Developer does not knowingly collect personal information from children.

## Data Retention

Because the Developer does not collect or store personal data, there is no remote data retention by the Developer.  
...
Contact: infodeskline@gmail.com

***

# 개인정보 처리방침

본 개인정보 처리방침은 thenovary(이하 "개발자")가 제공하는 모바일 애플리케이션 "모여봐요 도감" (영문명: NookDex, 일본어명: あつ森手帳)에 적용됩니다.

## 개요

본 애플리케이션은 동물의 숲: 모여봐요 동물의 숲 컬렉션 가이드 앱으로, 비공식 팬 제작 애플리케이션입니다.  
본 애플리케이션은 Nintendo와 제휴, 승인 또는 후원을 받지 않았습니다.  
관련 게임 자산, 캐릭터 및 상표의 모든 권리는 해당 권리자에게 있습니다.

본 애플리케이션은 회원가입을 요구하지 않으며, 자체 서버를 운영하지 않습니다.  
...
다만, 광고 및 인앱 결제 기능 제공을 위해 제3자 서비스를 사용하며, 이 과정에서 해당 서비스는 자체 개인정보처리방침에 따라 일부 정보를 자동으로 수집할 수 있습니다.
...
本プライバシーポリシーは、thenovary（以下「開発者」）が提供するモバイルアプリケーション「あつ森手帳」（英語名: NookDex、韓国語名: 모여봐요 도감）に適用されます。

## 概要

本アプリは「あつまれ どうぶつの森」のコレクションガイドアプリであり、非公式のファン制作アプリです。  
本アプリはNintendoと提携、承認、または後援関係にはありません。  
関連するゲーム資産、キャラクター、および商標の権利はすべて各権利者に帰属します。

本アプリはユーザー登録を必要とせず、独自のサーバーを運営しておりません。  
収集進捗、島情報、画像、設定など、ユーザーが作成したデータはすべて端末内にローカル保存されます。

## 情報の収集および利用

開発者は、ユーザーの個人情報を直接収集、保存、送信、または処理することはありません。

ただし、本アプリは広告配信およびアプリ内課金機能の提供のために第三者サービスを利用しており、各サービスはそれぞれのプライバシーポリシーに基づいて一定の情報を自動的に収集する場合があります。
...
ユーザーが撮影または選択した画像は端末内にローカル保存されます。  
開発者がこれらの画像を収集、アップロード、送信、外部保存、または閲覧することはありません。
...
収集進捗、島情報、設定などのユーザーデータはすべて端末内にローカル保存されます。  
